...
const ToggleButton = ({ targetElement }) => {
  const html = /*html*/ `
    <button type="button" class="text-black close-modal focus:outline-none focus:ring focus:border-blue-300">
      <svg class="h-4 fill-current text-gray-600 cursor-pointer text-opacity-60" viewBox="0 0 20 20">
  ...
  const newToggleButton = getNewComponent(targetElement, template);
